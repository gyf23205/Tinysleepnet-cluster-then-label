...
def train(
    ...
    if restart:  # delete former output dir and creat new one
    ...
    # Create logger
    logger = get_logger(log_file, level="info") # need to know how to read log
    ...
    # Further split training set as validation set (20%)
    n_valids = round(len(train_sids) * 0.20)
    ...
    print(train_files)
    # train_x, _, _, train_y = load_data(train_files, data_from_cluster=False)
    train_x, train_y, _, _ = load_data(train_files, data_from_cluster=False)
    ...
    # valid_x, valid_y, _, _ = load_data(valid_files, data_from_cluster=True)
    valid_x, valid_y, _, _ = load_data(valid_files)
    ...
    # test_x, test_y, _, _ = load_data(test_files, data_from_cluster=True)
    test_x, test_y, _, _ = load_data(test_files)
    ...
    for epoch in range(model.get_current_epoch(), config["n_epochs"]):
        ...
        if config['augment_signal']:
            ...
            for i in range(len(aug_train_x)):
                ...
                offset = np.random.uniform(-percent, percent) * aug_train_x[i].shape[1]
                roll_x = np.roll(aug_train_x[i], int(offset))
                # print("aug shape: ", aug_train_x[i].shape)
                # print("roll[:-1] shape: ", roll_x[:-1].shape)
                # print("type0: ", type(aug_train_x))
                # print("type1:", type(aug_train_x[i]))
                # print("type2:", type(roll_x[:-1]))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, default="./output/my_train")#！！！
    parser.add_argument("--restart", dest="restart", action="store_true")
    parser.add_argument("--no-restart", dest="restart", action="store_false")
    parser.add_argument("--log_file", type=str, default="./output/my_output.log")#！！！
