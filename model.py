...
class TinySleepNet(object):
    ...
    def __init__(
        ...
        if feature_extract:
            ...
            self.best_ckpt_path = config["feature_extract_model_dir"]
        else:
            self.best_ckpt_path = './out_sleepedf/feature_extract/best_ckpt'  # os.path.join(self.output_dir, "best_ckpt")
        ...
        if not testing:
            ...
            with tf.variable_scope("optimizer") as scope:
                update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)  # not in eager
                with tf.control_dependencies(update_ops):  # not in eager
                    # Pretraining
                    if not self.use_rnn:
    ...
    def evaluate(self, minibatches):
        ...
        if not self.use_rnn:
            for x, y in minibatches:
                ...
                logist.extend(outputs["test/logist"])
                trues.extend(y)
        else:
            for x, y, w, sl, re in minibatches:
                feed_dict = {
                ...
                tmp_trues = np.reshape(y, (self.config["batch_size"], self.config["seq_length"]))
                tmp_logits = np.reshape(outputs["test/logits"], (self.config["batch_size"], self.config["seq_length"],
                                                                 self.config["n_classes"]))

                for i in range(self.config["batch_size"]):
                    preds.extend(tmp_preds[i, :sl[i]])
                    trues.extend(tmp_trues[i, :sl[i]])
                    logist.extend(tmp_logits[i, :sl[i]])
    ...
    def reset_global_epoch(self):
        self.run(tf.assign(self.global_epoch, 0))
...
if __name__ == "__main__":
    ...
    model = TinySleepNet(config=pretrain, output_dir="./output/test", use_rnn=False)
