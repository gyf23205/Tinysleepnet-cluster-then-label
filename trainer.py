...
def run(db, gpu, from_fold, to_fold, suffix='', random_seed=42):
    ...
    os.environ["CUDA_VISIBLE_DEVICES"] = str(gpu) # specify gpu

    # Config file, basically equal to import the config module
    config_file = os.path.join('config', f'{db}.py') # config file path with string type
    ...
    for fold_idx in range(from_fold, to_fold+1):
        train(
            ...
            fold_idx=fold_idx,
            output_dir=os.path.join(output_dir, 'my_train'),
            log_file=os.path.join(output_dir, f'train0_{gpu}.log'),
            restart=True,
